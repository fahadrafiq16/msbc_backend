...
const sendPTEmail = require('../ptFormEmail.js');
...
const nonRecurringPaymentWebhook = async (req, res) => {
    ...
    try {
        ...
        if (payment.status === 'paid') {
            console.log('Paid');
            console.log(payment.metadata.userInfo);

            if (payment.metadata.userInfo.selectedOption.isRent) {
                console.log('PT Rent Page');
            }
            ...
            if (!payment.metadata.userInfo.selectedOption.isRent) {
                try {
                    ...
                    return res.send(`Email123 sent successfully to ${email}!`);

                } catch (error) {
                    console.error("Error sending email:", error);
                    return res.status(500).send("Failed to send email.");
                }
            } else if (payment.metadata.userInfo.selectedOption.isRent) {
                try {
                    sendPTEmail(
                        bedrijfsnaam,
                        kvknummer,
